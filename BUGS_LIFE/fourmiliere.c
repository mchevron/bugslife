...
//~ void fourmiliere_ouvriere_lecture_precontrol(unsigned i, unsigned j, 
											//~ char tab[MAX_LINE]) {
    //~ char check[MAX_LINE];
    //~ double test = 0.;
    //~ unsigned etape = 0;
    ...
    //~ if((strcmp(check, "FIN_LISTE") == ECHEC)
       //~ && (j < (p_fourmiliere+i)->nbO)) {
        //~ fourmi_etape_lecture(L_EXIT);
        //~ error_lecture_elements_fourmiliere(i, ERR_OUVRIERE, ERR_PAS_ASSEZ);   
    //~ }
    //~ if ((strcmp(check, "FIN_LISTE") == ECHEC) && ((p_fourmiliere+i)->nbF == 0))
        //~ fourmi_etape_lecture(L_COMPLETE);
    //~ if(strcmp(check, "FIN_LISTE") == ECHEC) {
        //~ fourmi_etape_lecture(L_GARDE);
    //~ }
    //~ if(j == (p_fourmiliere+i)->nbO && (p_fourmiliere+i)->nbO > 0) {
        //~ error_lecture_elements_fourmiliere(i, ERR_OUVRIERE, ERR_TROP);
        //~ fourmi_etape_lecture(L_EXIT);
    //~ }
    //~ if((p_fourmiliere+i)->nbF == 0) fourmi_etape_lecture(L_FOURMILIERE);
    //~ if((p_fourmiliere+i)->nbO == 0) {
        //~ j = 0;
        //~ fourmi_recoit_frml(p_fourmiliere);
        //~ etape = fourmi_garde_lecture(i, tab);
        //~ fourmi_etape_lecture(etape);
    //~ }
    //~ if(sscanf(tab, " %lf", &test) == ECHEC) fourmi_etape_lecture(L_OUVRIERE);
    //~ fourmi_etape_lecture(L_CONTINUE);
    //~ char check[MAX_LINE];
    //~ double test = 0.;
    //~ sscanf(tab, " %s", check);
    //~ if((strcmp(check, "FIN_LISTE") == ECHEC)
...
//~ void fourmiliere_garde_lecture_precontrol(unsigned i, unsigned j, char tab[MAX_LINE]){
    //~ char check[MAX_LINE];
    //~ double test = 0.;
    //~ sscanf(tab, " %s", check);
    //~ if((strcmp(check, "FIN_LISTE") == ECHEC)
    ...
    //~ }
    //~ if(strcmp(check, "FIN_LISTE") == ECHEC)
		//~ fourmi_etape_lecture(L_FOURMILIERE);
    //~ if(sscanf(tab, " %lf", &test)==ECHEC) 
		//~ fourmi_etape_lecture(L_GARDE);
    //~ fourmi_etape_lecture(L_CONTINUE);
    //~ char check[MAX_LINE];
    //~ double test = 0.;
